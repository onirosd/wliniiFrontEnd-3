        ...
        <div class="container-nueva-publicacion">
            <div class="responsive-nueva-publicacion">
                <w-card class="card-nueva-publicacion" :shadow="false">
                    <p class="subtitle bold mt-5 ml-5">Editar Publicación</p>
                    <p class="ml-5">Editar la información de la nueva publicación</p>
                    ...
                    <form @submit.prevent>
                        ...
                        <div>
                            ...
                            <w-input disabled v-model="form.Des_DireccionManual"></w-input>
                            <mapas-agregar
                                @eventAddress="eventDireccion($event)"
                                :isShowAddress="true"
                                :defaultMarker="coordenadas"></mapas-agregar>
                        </div>
                        <div class="mt-20">
                            <!-- FOTOS -->
                            <div class="f-header">
                            ...
                            <div class="flex flex-row justify-center">
                                <w-btn :dark="true" :large="true" @click="updatePublication">GUARDAR AVISO</w-btn>
                            </div>
                        </div>
                    </form>
                </w-card>
...
import { GetPublicationByID, UpdatePublication, UploadImages, RemoveImage } from "@/common/publication_apis";
...
export default {
    ...
    methods: {
        initPublicationData(data){
            if(data.IdTipoOperacion) this.tmpdata.tipooperacion = this.operaciontipos.find(e => e.IdTipoOperacion == data.IdTipoOperacion).Descripcion;
            if(data.IdTipoInmueble) this.tmpdata.tipoinmueble = this.inmuebletipos.find(e => e.IdTipoInmueble == data.IdTipoInmueble).Descripcion;
            if(data.IdUbigeo){
                let location = this.locations.find(e => e.IdUbigeo === data.IdUbigeo);
                this.tmpdata.departamento = location.Departamento;
                this.tmpdata.provincias = location.Provincia;
                this.tmpdata.distrito = location.Distrito;
            }
            if(data.IdTipoMoneda) this.tmpdata.moneda = this.monedatipos.find(e => e.IdTipoMoneda == data.IdTipoMoneda).Descripcion;
            if(data.IdTipoComision) this.tmpdata.comision = this.comisiontipos.find(e => e.IdTipoComision == data.IdTipoComision).Descripcion;
            if(data.Des_Coordenadas) this.coordenadas = JSON.parse(data.Des_Coordenadas);

            let images = data.images; delete data.images;
            images.length && images.map(i => {
                this.resource.photos.push({
                    file: null,
                    url: SERVER_URL + i.Des_url,
                    id: i.IdPubImage
                })
            });

            delete data.IdPubCabecera;
            delete data.FechaCreacion;
            delete data.FechaModificacion;
            delete data.UsuarioCreacion;
        ...
        eventDireccion(params) {
            this.form.Des_DireccionManual = params.direccion;
        ...
        removePhoto(file) {
            if(file.file === null && file.id){
                RemoveImage(file.id).then(res => {
                    ...
                    notify('error', null, 'no se puede eliminar la imagen');
                });
            }
        ...
        updatePublication() {
            ...
            UpdatePublication(this.publication_id, {data: this.form}).then(res => {
                if(res && res.data) {
        ...
        uploadImages(){
            ...
            formData.append("publication_id", this.publication_id);
            ...
            UploadImages(formData).then(res => {
                ...
                else notify('error', null, 'La carga de la imagen falló');
            }).catch(err => {
                this.submitting = false;
